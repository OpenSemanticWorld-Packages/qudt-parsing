...
def load_qudt_dump(url: str) -> dict:
    ...
    qudt_context = {
        ...
        # note: define aliases if you wish to make the dict more readable
        # "id": "@id",
        # "type": "@type",
        # "label": "rdfs:label",
        # "hasFactorUnit": {"@id": "qudt:hasFactorUnit", "@type": "@id"},
    }

    # load the graph as json-ld dict and compact with the given context
...
PREFIXES = [
    "Atto", "Centi", "Deca", "Deci", "Deka", "Exa", "Exbi", "Femto", "Gibi", "Giga",
    "Hecto", "Kibi", "Kilo", "Mebi", "Mega", "Micro", "Milli", "Nano", "Pebi", "Peta",
    "Pico", "Quecto", "Quetta", "Ronna", "Ronto", "Tebi", "Tera", "Yobi", "Yocto",
...
if __name__ == "__main__":
    ...
    iri_dict = {}
    # @id =  "http://qudt.org/vocab/unit/" + unit name
    # Iterate over the @graph and filter for @id
    ...
    # Resolve hasFactorUnit references to actual unit definitions
    for item in graph["@graph"]:
        if not "qudt:Unit" in item.get("@type", []):
            continue
    ...
    for unit in type_dict.get("qudt:Unit", []):
        ...
        if "qudt:prefix" in unit:
            ...
            for candidate in type_dict.get("qudt:Unit", []):
                if base_unit_name in candidate.get("@id", ""):
                    ...
                    _logger.info(" - Base Non-prefixed unit found: %s", base_unit_name)
                    break
            ...
            if not base_unit_found and "qudt:scalingOf" not in unit:
                ...
                unit_type_dict["Prefixed, non-composed unit with no Non-prefixed base"].append(unit)
                _logger.info(" - (also listed as Prefixed, non-composed Unit with no Non-prefixed base)")

        # Non-prefixed units
        if "qudt:prefix" not in unit:
            unit_type_dict["Units without a prefix statement"].append(unit)
            _logger.info("Non-prefixed Unit found: %s - Label: %s",
                            unit_id, label)
            # Non-prefixed non-composed unit
            if  "qudt:hasFactorUnit" not in unit:
                unit_type_dict["Non-prefixed, non-composed unit"].append(unit)
                _logger.info("Non-prefixed, non-composed Unit found: %s - Label: %s",
                             unit_id, label)
        # Composed units
        if "qudt:hasFactorUnit" in unit:
            ...
            if prefixed:
                ...
                base_unit_found = False
                for candidate in type_dict.get("qudt:Unit", []):
                    if base_unit_name in candidate.get("@id", "") and "qudt:prefix" not in candidate:
                        base_unit_found = True
                        if base_unit_name not in base_units_to_prefixed_composed_units:
                            base_units_to_prefixed_composed_units[base_unit_name] = []
                        base_units_to_prefixed_composed_units[base_unit_name].append(unit["@id"])
                        _logger.info(" - Base Non-prefixed unit found: %s",
                                     base_unit_name)
                        break
                if "qudt:scalingOf" not in unit:
                    unit_type_dict[
                        "Prefixed, composed unit with missing scalingOf"].append(unit)
                    _logger.info(
                        " - (also listed as Prefixed, composed unit with missing scalingOf)")
                if base_unit_found and "qudt:scalingOf" not in unit:
                    unit_type_dict[("Prefixed, composed unit with missing "
                                    "scalingOf but base unit available")].append(unit)
                    _logger.info(" - (also listed as Prefixed, composed unit with missing "
                                    "scalingOf but base unit available)")
                if not base_unit_found:
                    unit_type_dict["Prefixed, composed unit with no Non-prefixed base"].append(unit)
                    _logger.info(" - (also listed as Prefixed, composed Unit with no Non-prefixed base)")
            # Non-prefixed composed unit
            else:
                unit_type_dict["Units without a prefix statement"].append(unit)
    ...
    for type_name, items in unit_type_dict.items():
        ...
        _logger.info(" - Type: %s - Count: %d", type_name, len(items))
    _logger.info(
        f"Number of prefixed composed units with their Non-prefixed base units: {len(base_units_to_prefixed_composed_units)}")
    ...
    # Estimate the count of units that are listed in multiple lists of the unit_type-dict
    # Create sets of the units listed in each type, use the @id as unique id
    ...
    # Handling of quantity kinds and their applicable units
